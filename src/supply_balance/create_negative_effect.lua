...
function Flexible_unit_caps:create_negative_sb_effect(faction, supply_balance)
  ...
  local num_of_armies = Flexible_unit_caps:get_armies_count(force_list)
  ...
  local supply_balance_effect_bundle = cm:create_new_custom_effect_bundle(supply_balance_bundle_name);
	supply_balance_effect_bundle:add_effect("wh_main_effect_force_all_campaign_recruitment_cost_all", "faction_to_force_own", effect_strenght);
