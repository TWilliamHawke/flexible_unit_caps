...
---@param caches {}
function Flexible_unit_caps:set_tooltip_for_unit_in_queue(component, queued_units, caches)
  ...
  local supply_text, unit_cost = self:construct_unit_supply_text(unit_name, "fluc_unit_consume_future", function(unit_group)
    ...
    local _, unit_index_in_queue = self:get_units_count_from_cache(caches.queued_units_cache, unit_group,
      component_name);
    local unit_index = units_in_army + unit_index_in_queue;
    local group_capacity = self:get_unit_cap_from_cache(caches.queued_units_cache, unit_group);
